...
- `effectMagnitudeByLevel` has no die-type column. Five effects (Bashing, all 24 Bane creature types, all 16 Damage(X) elemental/alignment types via both delivery types, Shield Spikes, Vampirism) are dice-based — the stored magnitude is the die *count* only (die size: d6 for all of them except Bane which is d10, and Vampirism which is d2). Matters if magnitude ever needs to drive real damage math instead of just display/comparison.
